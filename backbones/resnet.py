...
class BasicBlock(nn.Cell):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None, norm_cfg=None, act_cfg=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=dilation, dilation=dilation, pad_mode='pad',has_bias=False)
...
class Bottleneck(nn.Cell):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None, norm_cfg=None, act_cfg=None):
        ...
        # self.stride = stride
        # self.dilation = dilation
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, stride=1, padding=0, pad_mode='pad',has_bias=False)
        self.bn1 = BuildNormalization(constructnormcfg(placeholder=planes, norm_cfg=norm_cfg))
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=dilation, dilation=dilation, pad_mode='pad',has_bias=False)
        self.bn2 = BuildNormalization(constructnormcfg(placeholder=planes, norm_cfg=norm_cfg))
        self.conv3 = nn.Conv2d(planes, planes * self.expansion, kernel_size=1, stride=1, padding=0, pad_mode='pad',has_bias=False)
        self.bn3 = BuildNormalization(constructnormcfg(placeholder=planes * self.expansion, norm_cfg=norm_cfg))
        self.relu = BuildActivation(act_cfg)
        self.downsample = downsample
        self.stride = stride
    ...
    def __init__(self, in_channels=3, base_channels=64, stem_channels=64, depth=101, outstride=8, contract_dilation=True, use_stem=True, 
        ...
        if use_stem:
            ...
            self.stem = nn.SequentialCell(
                nn.Conv2d(in_channels, stem_channels // 2, kernel_size=3, stride=2, padding=1, pad_mode='pad',has_bias=False),
                BuildNormalization(constructnormcfg(placeholder=stem_channels // 2, norm_cfg=norm_cfg)),
                BuildActivation(act_cfg),
                nn.Conv2d(stem_channels // 2, stem_channels // 2, kernel_size=3, stride=1, padding=1, pad_mode='pad',has_bias=False),
                BuildNormalization(constructnormcfg(placeholder=stem_channels // 2, norm_cfg=norm_cfg)),
                BuildActivation(act_cfg),
                nn.Conv2d(stem_channels // 2, stem_channels, kernel_size=3, stride=1, padding=1, pad_mode='pad',has_bias=False),
                BuildNormalization(constructnormcfg(placeholder=stem_channels, norm_cfg=norm_cfg)),
                BuildActivation(act_cfg),
            )
        else:
            self.conv1 = nn.Conv2d(in_channels, stem_channels, kernel_size=7, stride=2, padding=3,pad_mode='pad',has_bias=False)
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            if use_avg_for_downsample:
                downsample = nn.SequentialCell(
                    nn.AvgPool2d(kernel_size=stride, stride=stride, ceil_mode=True, count_include_pad=False, pad_mode='pad'),
                    nn.Conv2d(inplanes, planes * block.expansion, kernel_size=1, stride=1, padding=0, pad_mode='pad',has_bias=False),
                    BuildNormalization(constructnormcfg(placeholder=planes * block.expansion, norm_cfg=norm_cfg))
                )
            else:
                downsample = nn.SequentialCell(
                    nn.Conv2d(inplanes, planes * block.expansion, kernel_size=1, stride=stride, padding=0,pad_mode='pad', has_bias=False),
                    BuildNormalization(constructnormcfg(placeholder=planes * block.expansion, norm_cfg=norm_cfg))
                )
        layers = []
