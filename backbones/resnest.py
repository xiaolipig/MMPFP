...
class RSoftmax(nn.Cell):
    ...
    def construct(self, x):
        ...
        if self.radix > 1:
            ...
            x = x.view(batch, self.groups, self.radix, -1).swapaxes(1, 2)
            # x = F.softmax(x, dim=1)
...
class SplitAttentionConv2d(nn.Cell):
    def __init__(self, in_channels, channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, radix=2, reduction_factor=4, norm_cfg=None, act_cfg=None):
        ...
        self.radix = radix
        self.conv = nn.Conv2d(in_channels, channels * radix, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, group=groups * radix, has_bias=False, pad_mode='pad')
        ...
        self.fc1 = nn.Conv2d(channels, inter_channels, kernel_size=1, stride=1, padding=0, group=groups, pad_mode='pad')
        self.bn1 = BuildNormalization(constructnormcfg(placeholder=inter_channels, norm_cfg=norm_cfg))
        self.fc2 = nn.Conv2d(inter_channels, channels * radix, kernel_size=1, stride=1, padding=0, group=groups, pad_mode='pad')
    ...
    def construct(self, x):
        ...
        if self.radix > 1:
            ...
            gap = splits.sum(axis=1)
        else:
            gap = x
        # gap = F.adaptive_avg_pool2d(gap, 1)
        gap = ops.adaptive_avg_pool2d(gap, 1)
        gap = self.fc1(gap)
        gap = self.bn1(gap)
        gap = self.relu(gap)
        atten = self.fc2(gap)
        ...
        if self.radix > 1:
            ...
            out = ops.sum(attens * splits, dim=1)
        else:
            out = atten * x
...
class Bottleneck(_Bottleneck):
    ...
    def __init__(self, inplanes, planes, groups=1, base_width=4, base_channels=64, radix=2, reduction_factor=4, use_avg_after_block_conv2=True,
        ...
        self.conv1 = nn.Conv2d(inplanes, width, kernel_size=1, stride=1, padding=0, has_bias=False, pad_mode='pad')
        ...
        delattr(self, 'bn2')
        self.conv3 = nn.Conv2d(width, planes * self.expansion, kernel_size=1, stride=1, padding=0, has_bias=False, pad_mode='pad')
        ...
        if self.use_avg_after_block_conv2: 
            self.avg_layer = nn.AvgPool2d(3, self.stride, padding=1, pad_mode='pad')
    '''forward'''
    def construct(self, x):
        identity = x
        out = self.conv1(x)
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            if use_avg_for_downsample:
                downsample = nn.SequentialCell(
                    ...
                    nn.Conv2d(inplanes, planes * block.expansion, kernel_size=1, stride=1, padding=0, has_bias=False, pad_mode='pad'),
                    BuildNormalization(constructnormcfg(placeholder=planes * block.expansion, norm_cfg=norm_cfg))
                )
            else:
                downsample = nn.SequentialCell(
                    nn.Conv2d(inplanes, planes * block.expansion, kernel_size=1, stride=stride, padding=0, has_bias=False, pad_mode='pad'),
                    BuildNormalization(constructnormcfg(placeholder=planes * block.expansion, norm_cfg=norm_cfg))
                )
        layers = []
