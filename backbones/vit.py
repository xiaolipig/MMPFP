...
# from bricks import BuildNormalization, MultiheadAttention, PatchEmbed, FFN, constructnormcfg
# from .bricks._legacy.nn.dropout import Dropout
...
class TransformerEncoderLayer(nn.Cell):
    def __init__(self, embed_dims, num_heads, feedforward_channels, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., num_fcs=2, 
        ...
        # embed_dims = embed_dims if isinstance(embed_dims, tuple) else (embed_dims,)
        self.ln1 = BuildNormalization(constructnormcfg(placeholder=embed_dims if isinstance(embed_dims, tuple) else (embed_dims, ), norm_cfg=norm_cfg))
        ...
        self.ln2 = BuildNormalization(constructnormcfg(placeholder=embed_dims if isinstance(embed_dims, tuple) else (embed_dims, ), norm_cfg=norm_cfg))
        ffn_cfg.update(dict(
            embed_dims=embed_dims,
...
class VisionTransformer(nn.Cell):
    ...
    #              patch_norm=False, final_norm=False, interpolate_mode='bicubic', num_fcs=2, use_checkpoint=False, batch_size=None):
    def __init__(self, img_size=224, patch_size=16, in_channels=3, embed_dims=768, num_layers=12, num_heads=12, mlp_ratio=4, out_indices=-1, qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., with_cls_token=True, output_cls_token=False, norm_cfg=None, act_cfg=None, 
        ...
        self.cls_token = Parameter(Tensor(np.zeros((1, 1, embed_dims)), mindspore.float32),name='cls_token')
        self.pos_embed = Parameter(Tensor(np.zeros((1,num_patches + 1, embed_dims)), mindspore.float32), name='pos_embed')
        # self.drop_after_pos = nn.Dropout(keep_prob=1.0-drop_rate)
        ...
        dpr = [x.item(0) for x in ops.linspace(mindspore.Tensor(0, mindspore.float32), mindspore.Tensor(drop_path_rate, mindspore.float32), num_layers)]
    ...
    @staticmethod
    def resizeposembed(pos_embed, input_shape, pos_shape, mode):
        ...
        pos_embed_weight = ops.interpolate(pos_embed_weight, None, None, sizes=input_shape, mode='bilinear', coordinate_transformation_mode='align_corners')
        # pos_embed_weight = torch.flatten(pos_embed_weight, 2).transpose(1, 2)
        pos_embed_weight = pos_embed_weight.reshape(pos_embed_weight.shape[0], pos_embed_weight.shape[1], -1)
        pos_embed_weight = pos_embed_weight.swapaxes(1, 2)
        # pos_embed_weight = torch.flatten(pos_embed_weight, 2).transpose(1, 2)

        # pos_embed = torch.cat((cls_token_weight, pos_embed_weight), dim=1)
        con_op = ops.Concat(axis=1)
        pos_embed = con_op((cls_token_weight, pos_embed_weight))
        return pos_embed
    '''forward'''
