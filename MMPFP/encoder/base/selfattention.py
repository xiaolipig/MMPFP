    ...
    def buildproject(self, in_channels, out_channels, num_convs, use_norm, norm_cfg, act_cfg):
        if use_norm:
            ...
            for _ in range(num_convs - 1):
                convs.append(nn.SequentialCell(
                    nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, padding=0, has_bias=False, pad_mode='pad'),
                    BuildNormalization(constructnormcfg(placeholder=out_channels, norm_cfg=norm_cfg)),
                    BuildActivation(act_cfg),
                ))
        else:
            convs = [nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, has_bias=False, pad_mode='pad')]
            for _ in range(num_convs - 1):
                convs.append(
                    nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, padding=0, has_bias=False, pad_mode='pad')
                )
        if len(convs) > 1: return nn.SequentialCell(*convs)
        return convs[0]